<template>
    <div>
      ...
      <div class="table-area-wrapper order-table-content">
        <v-data-table v-model="selected" :headers="headers" :items="desserts" :page.sync="page"
          ...
          :search="search" class="elevation-1" hide-default-footer :items-per-page="itemsPerPage">
          ...
          <template v-slot:item.last_update>
            <td class="last-update">
                    ...
                    <span class="time-only px-4">১০:২৬</span>
                  </td>
          </template>
          <template v-slot:item.responsible>
  ...
  <script>
    export default {
      name: "ProductUpdateHistoryTable",
